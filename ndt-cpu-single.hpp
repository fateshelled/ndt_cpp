...
struct ndtpoint2 {
    ...
    ndtcpp::mat2x2 cov;
    ndtpoint2() = default;
    ndtpoint2(const ndtcpp::point2& mean, const ndtcpp::mat2x2& cov) : mean(mean), cov(cov) {}
};
...
inline void compute_covariances(std::vector<ndtcpp::point2>& points, std::vector<ndtpoint2> &results, const size_t N = 10){
    ...
    for(std::size_t i = 0; i < point_size; i++) {
        ...
        // const auto mean = compute_mean(result_points);
        // const auto cov = compute_covariance(result_points, mean);
        const auto cov = compute_covariance(result_points, points[i]);
        results[i] = {points[i], cov};
    }
}
